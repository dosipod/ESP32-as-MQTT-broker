/**
 * MQTT broker with WiFiManager  advanced demo, contains advanced configurartion options
...
class MQTT: public PicoMQTT::Server {
    protected:
        PicoMQTT::ConnectReturnCode auth(const char * client_id, const char * username, const char * password) override {
            ...
            if (
                ((String(username) == "mqttd1") && (String(password) == "abcd1234"))
                || ((String(username) == "mqttd2") && (String(password) == "abcd1234"))) {
...
void loop() {
  ...
  mqtt.loop();
  if (random(50000) == 0){ mqtt.publish("picomqtt/welcome", "Hello from PicoMQTT!"); }
}
